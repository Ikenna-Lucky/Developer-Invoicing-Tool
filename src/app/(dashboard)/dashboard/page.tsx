...
import { Plus, ArrowRight, TrendingUp, AlertTriangle, Users, Zap } from "lucide-react";
...
function CollectionRing({ rate, triggered }: { rate: number; triggered: boolean }) {
  const R    = 54;
  ...
  return (
    <svg width="144" height="144" viewBox="0 0 144 144">
      ...
      <text x="72" y="66" textAnchor="middle" dominantBaseline="middle"
        fill="white" fontSize="24" fontWeight="700" fontFamily="Inter, sans-serif"
        style={{ letterSpacing: "-0.04em" }}>
        {displayRate}%
      </text>
      <text x="72" y="86" textAnchor="middle"
        fill="rgba(255,255,255,0.3)" fontSize="11" fontFamily="Inter, sans-serif"
...
function FadeUp({ children, delay = 0, show }: {
...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [invJson, cliJson] = await Promise.all([invRes.json(), cliRes.json()]);
  ...
  const outstanding    = invoices.filter(i => i.status === "sent").reduce((s, i) => s + Number(i.totalAmount), 0);
  ...
  const animCollected = useCountUp(show ? Math.round(totalCollected) : 0, 1400, 0);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <div className="space-y-2">
          <div className="h-3.5 w-24 rounded-md animate-pulse" style={{ background: "rgba(255,255,255,0.07)" }} />
          <div className="h-7 w-40 rounded-md animate-pulse" style={{ background: "rgba(255,255,255,0.07)" }} />
        </div>
      </div>
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 h-52 rounded-2xl animate-pulse" style={{ background: "#161b27" }} />
        <div className="h-52 rounded-2xl animate-pulse" style={{ background: "#161b27" }} />
      </div>
      <div className="grid grid-cols-3 gap-4">
        {[...Array(3)].map((_, i) => <div key={i} className="h-28 rounded-2xl animate-pulse" style={{ background: "#161b27" }} />)}
      </div>
      <div className="h-64 rounded-2xl animate-pulse" style={{ background: "#161b27" }} />
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <FadeUp show={show} delay={0}>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <h1 className="text-[26px] font-bold text-white tracking-tight">Overview</h1>
          </div>
          <Link
            href="/invoices/create"
            className="inline-flex items-center gap-2 text-white font-semibold text-[14px] px-5 py-2.5 rounded-xl"
            style={{
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        {/* ── HERO CARD ── */}
        <FadeUp show={show} delay={60}>
          <div
            className="col-span-2 rounded-2xl p-7 relative overflow-hidden h-full"
            style={{
              background: "linear-gradient(145deg, #12172a 0%, #181f33 55%, #111826 100%)",
              border: "1px solid rgba(255,255,255,0.08)",
            }}
          >
            {/* Top shimmer line */}
            <div className="absolute top-0 left-0 right-0 h-px"
              style={{ background: "linear-gradient(90deg, transparent 0%, rgba(74,222,128,0.5) 40%, rgba(96,165,250,0.4) 70%, transparent 100%)" }} />

            {/* Background glow — positioned bottom-left */}
            ...
            <div className="flex items-start gap-1.5 mb-5">
              <span className="text-[18px] font-semibold mt-3 leading-none"
                style={{ color: "rgba(74,222,128,0.5)" }}>
                $
              </span>
              <span
                className="text-[58px] font-extrabold leading-none tracking-[-0.04em]"
                style={{
            ...
            {totalInvoiced > 0 ? (
                ...
                <div className="h-[3px] rounded-full overflow-hidden mb-2"
                  ...
                  <div
                    ...
                    style={{
                      ...
                      transition: "width 1.5s cubic-bezier(0.22, 1, 0.36, 1) 200ms",
                      boxShadow: "0 0 8px rgba(74,222,128,0.5)",
                    }}
                  />
                </div>
                <p className="text-[12px] text-white/25">
                  <span className="text-white/50 font-semibold">{Math.round(collectionRate)}%</span>
              ...
              <div className="mb-6">
                ...
                <p className="text-[12px] text-white/20 mt-2">No invoices issued yet</p>
              </div>
            )}
            ...
            <div className="flex items-center gap-7">
              <div>
                <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.18em] mb-1">En Route</p>
                <p className="text-[20px] font-bold text-white/70 font-mono tracking-tight">
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: "rgba(248,113,113,0.55)" }}>
                        Past Due
                      </p>
                    </div>
                    <p className="text-[20px] font-bold font-mono tracking-tight" style={{ color: "#f87171" }}>
                      {formatCurrency(overdueAmt)}
                    </p>
                    <p className="text-[11px] mt-0.5" style={{ color: "rgba(248,113,113,0.4)" }}>
        ...
        <FadeUp show={show} delay={140}>
          ...
          >
            ...
            <p className="text-[12px] text-white/30 mt-3 text-center relative z-10 leading-relaxed">
              ...
              invoice{invoices.length !== 1 ? "s" : ""} paid
            </p>
          </div>
        </FadeUp>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <FadeUp show={show} delay={220}>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <p className="text-[10px] font-bold text-white/25 uppercase tracking-[0.18em]">Pipeline</p>
              <div className="w-6 h-6 rounded-md flex items-center justify-center"
                style={{ background: "rgba(96,165,250,0.12)" }}>
                <TrendingUp size={12} className="text-blue-400" />
              </div>
            </div>
            <p className="text-[26px] font-bold text-white font-mono tracking-tight mb-1">
              {formatCurrency(outstanding)}
            </p>
        ...
        <FadeUp show={show} delay={300}>
          <div
            ...
            style={{
              ...
              border:     overdueCount > 0 ? "1px solid rgba(239,68,68,0.18)" : "1px solid rgba(255,255,255,0.07)",
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-1.5">
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.18em]"
                  style={{ color: overdueCount > 0 ? "rgba(248,113,113,0.6)" : "rgba(255,255,255,0.25)" }}>
                  Needs Attention
                </p>
              </div>
              <div className="w-6 h-6 rounded-md flex items-center justify-center"
                style={{ background: overdueCount > 0 ? "rgba(239,68,68,0.12)" : "rgba(255,255,255,0.05)" }}>
                <AlertTriangle size={12}
                  style={{ color: overdueCount > 0 ? "#f87171" : "rgba(255,255,255,0.2)" }} />
              </div>
            </div>
            <p className="text-[26px] font-bold font-mono tracking-tight mb-1"
              style={{ color: overdueCount > 0 ? "#f87171" : "rgba(255,255,255,0.2)" }}>
              {overdueCount > 0 ? formatCurrency(overdueAmt) : "—"}
            </p>
            <p className="text-[12px]"
              style={{ color: overdueCount > 0 ? "rgba(248,113,113,0.45)" : "rgba(255,255,255,0.2)" }}>
              {overdueCount > 0
        ...
        <FadeUp show={show} delay={380}>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <p className="text-[10px] font-bold text-white/25 uppercase tracking-[0.18em]">Client Base</p>
              <div className="w-6 h-6 rounded-md flex items-center justify-center"
                style={{ background: "rgba(167,139,250,0.12)" }}>
                <Users size={12} className="text-purple-400" />
              </div>
            </div>
            <p className="text-[26px] font-bold text-white font-mono tracking-tight mb-1">{clients}</p>
            <p className="text-[12px] text-white/30">
              {invoices.length} invoice{invoices.length !== 1 ? "s" : ""} issued in total
            </p>
          </div>
        </FadeUp>
      </div>
        ...
        >
          ...
          {recentInvoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <p className="text-[14px] font-semibold text-white/60 mb-1">Nothing here yet</p>
              <p className="text-[13px] text-white/25 mb-5">Your first invoice will show up right here.</p>
              <Link
                href="/invoices/create"
              ...
              {recentInvoices.map((inv, idx) => {
                ...
                return (
                  <div
                    ...
                    className="px-6 py-4 grid grid-cols-[1fr_150px_130px_110px] items-center gap-4 cursor-pointer"
                    style={{
                      borderBottom: idx < recentInvoices.length - 1
